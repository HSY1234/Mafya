...
public class JwtAuthenticationFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        // 헤더에서 JWT 를 받아옵니다.
        String accessToken = tokenProvider.resolveAccessToken(req);
        String refreshToken = tokenProvider.resolveRefreshToken((req));
        String requestURI = req.getRequestURI();

        // 유효한 토큰인지 확인합니다.
        if (accessToken != null && refreshToken != null && tokenProvider.validateToken(accessToken).equals("valid") && tokenProvider.validateToken(refreshToken).equals("valid")) {
